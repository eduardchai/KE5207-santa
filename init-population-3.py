        ...
        while trip_total_weight < SLEIGH_CAPACITY and not end_trip:
            ...
            for point in closest_5k_points:
                ...
                if count < 5000:
                    ...
                    if len(trip_gifts_list) < 30:
                        curr_point = json.loads(REDIS_CLIENT.hget('point-lookup', f'gift-{gift_id}'))
                        curr_point = (float(curr_point['latitude']), float(curr_point['longitude']))

                        distance_to_north_pole = haversine(curr_point, (NORTH_POLE_LAT, NORTH_POLE_LONG))
                        # randomized available gifts
                        random.shuffle(available_gifts)

                        found_next = False
                        print(f'available_gifts length:{len(available_gifts)}')
                        for next_gift_id in available_gifts:
                            next_weight = float(REDIS_CLIENT.hget('weight-lookup', f'gift-{next_gift_id}'))

                            # get points coordinate
                            next_point = json.loads(REDIS_CLIENT.hget('point-lookup', f'gift-{next_gift_id}'))
                            next_point = (float(next_point['latitude']), float(next_point['longitude']))

                            # calculate the distance
                            next_distance = haversine(curr_point, next_point)
                            if (trip_total_weight + next_weight <= SLEIGH_CAPACITY) and (next_distance < distance_to_north_pole):
